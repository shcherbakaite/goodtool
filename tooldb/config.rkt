...
(current-locale-specifier 'tooldb)
...
(current-option-name-prefix "TOOLDB")
...
(define-values (_ default-db-host default-db-port default-db-name default-db-username default-db-password)
  (parse-database-url (or (getenv "DATABASE_URL") "postgres://tooldb:tooldb@127.0.0.1:5432/tooldb")))
